...
for img_ruta in imagenes_ruta:
    ...
    # 5. Mostrar imágenes y resultados intermedios (media, covarianza)
    if len(pixeles) > 0:
        media_pixeles = np.mean(pixeles, axis=0)
        X_train.append(media_pixeles)
        y_train.append(1)  # Etiqueta para los colores de interés
...
nueva_imagen = cv2.imread('Prueba1.jpg')  # Cargar nueva imagen
