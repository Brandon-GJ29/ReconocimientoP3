...
for img_ruta in imagenes_ruta:
    ...
    # Definir límites de color amarillo para segmentación
    limite_inferior = np.array([20, 100, 100], dtype=np.uint8)  
    limite_superior = np.array([30, 255, 255], dtype=np.uint8)  

    # Crear máscara para los píxeles que cumplen con el límite
    mascara_amarillos = cv2.inRange(imagen_hsv, limite_inferior, limite_superior)

    # Extraer los píxeles correspondientes al color amarillo
    pixeles_amarillos = imagen_hsv[mascara_amarillos != 0]

    # Calcular la media de los píxeles extraídos
    if len(pixeles_amarillos) > 0:
        media_pixeles = np.mean(pixeles_amarillos, axis=0)
        X_train.append(media_pixeles)
        y_train.append(1)  # Etiqueta para amarillo
        
        # Calcular y mostrar la matriz de covarianza
        covarianza = np.cov(pixeles_amarillos, rowvar=False)
        ...
        plt.subplot(1, 3, 2)
        plt.imshow(mascara_amarillos, cmap='gray')
        plt.title('Máscara de Amarillos')
...
# Aplicar el mismo límite de color amarillo
...
# Asignar valores de gris: 0 para fondo, 255 para objeto de interés (amarillo)
clases_imagen[mascara_nueva != 0] = 255  # Amarillo
